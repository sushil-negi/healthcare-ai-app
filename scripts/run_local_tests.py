...
def main():
    ...
    if not run_command(
        "python3 scripts/create_mock_model.py", "Create mock healthcare model"
    ):
        return False
    ...
    run_e2e = (
        input("Do you want to start Docker services and run E2E tests? (y/N): ")
        .lower()
        .startswith("y")
    )
    ...
    if run_e2e:
        ...
        if all_healthy:
            ...
            result = subprocess.run(
                ["python3", "-m", "pytest", "tests/e2e/", "-v", "--tb=short"], env=env
            )
